...
export class SorobanClientService {
  ...
  async getEscrow(escrowId: number): Promise<any> {
    try {
      ...
      const key = StellarSdk.xdr.ScVal.scvVec([
        ...
        StellarSdk.xdr.ScVal.scvU64(new StellarSdk.xdr.Uint64(escrowId.toString())),
      ]);
      ...
      this.logger.error(`Error fetching escrow ${escrowId}: ${error.message}`);
      return null;
    }
  }
   ...
   */
  private decodeEscrowScVal(scVal: StellarSdk.xdr.ScVal): any {
    ...
    map.forEach((entry) => {
      ...
      if (key === 'status') {
        ...
        result.recipient = StellarSdk.Address.fromScAddress(val.address()).toString();
      }
    });
